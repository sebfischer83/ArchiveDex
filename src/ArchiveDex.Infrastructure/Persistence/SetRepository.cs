...
{
    public class SetRepository(ArchiveDexDbContext db) : ISetRepository
    {
        ...
        {
            _ = _db.CardSetExternalIds.Add(externalId);
            _ = await _db.SaveChangesAsync(ct);
        }
        ...
        {
            SetMapping? existing = await _db.SetMappings.FirstOrDefaultAsync(
                m => m.Source == source && m.Language == language && m.ExternalId == externalId, ct);

            DateTime now = DateTime.UtcNow;
            if (existing is null)
            {
                _ = _db.SetMappings.Add(new SetMapping
                {
                    Id = Guid.NewGuid(),
                    CardSetId = cardSetId,
                    Source = source,
        ...
        {
            if (fromCardSetId == toCardSetId)
            {
                return;
            }

            _ = await _db.CardSetExternalIds.Where(x => x.CardSetId == fromCardSetId)
                .ExecuteUpdateAsync(s => s.SetProperty(x => x.CardSetId, toCardSetId), ct);

            _ = await _db.CardPrints.Where(c => c.CardSetId == fromCardSetId)
                .ExecuteUpdateAsync(s => s.SetProperty(c => c.CardSetId, toCardSetId), ct);

            _ = await _db.SetMappings.Where(m => m.CardSetId == fromCardSetId)
                .ExecuteUpdateAsync(s => s.SetProperty(m => m.CardSetId, toCardSetId), ct);

            _ = await _db.SetRelations.Where(r => r.SourceSetId == fromCardSetId)
                .ExecuteUpdateAsync(s => s.SetProperty(r => r.SourceSetId, toCardSetId), ct);
            _ = await _db.SetRelations.Where(r => r.TargetSetId == fromCardSetId)
            ...
            _ = await _db.SetRelations.Where(r => r.SourceSetId == r.TargetSetId).ExecuteDeleteAsync(ct);
            _ = await _db.PendingSetMappings.Where(p => p.SuggestedCardSetId == fromCardSetId)
                .ExecuteUpdateAsync(s => s.SetProperty(p => p.SuggestedCardSetId, toCardSetId), ct);

            _ = await _db.CardSets.Where(s => s.Id == fromCardSetId).ExecuteDeleteAsync(ct);
        }
