...
{
    public class TcgdexClient(HttpClient httpClient, SupportedLanguage language = SupportedLanguage.en, string baseUrl = "https://api.tcgdex.net/v2")
    {
        ...
        {
            var path = set is not null ? new[] { "sets", set, id } : ["cards", id];
        ...
        public async Task<List<CardResume>?> FetchCardsAsync(string? set = null, CancellationToken ct = default)
        {
            if (set is not null)
            {
                Set? fSet = await FetchAsync<Set>(["sets", set], null, ct).ConfigureAwait(false);
                return fSet?.Cards;
            }
        ...
        public async Task<List<SetResume>?> FetchSetsAsync(string? serie = null, CancellationToken ct = default)
        {
            ...
            {
                Serie? fSerie = await FetchAsync<Serie>(["series", serie], null, ct).ConfigureAwait(false);
                return fSerie?.Sets;
            }
            ...
            where T : class
        {
            if (path.Length == 0)
            {
                throw new ArgumentException("Endpoint path is empty.");
            }
            ...
            return !ValidEndpoints.Contains(endpoint)
                ...
                : ExecuteRequestAsync<T>(BuildUrl(path, query), ct);
        }
