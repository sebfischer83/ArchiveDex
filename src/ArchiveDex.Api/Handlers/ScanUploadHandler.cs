...
public static class ScanUploadHandler
{
        ...
        CancellationToken ct)
    {
        ...
        var imageAsset = await imageStore.StoreAsync(stream, image.FileName, ct);
        ...
        var ocrResult = await ocrEngine.ProcessAsync(scanJob.Id, imageAsset.RelativePath, cardLanguageHint, ct);
        ocrResult.ScanJobId = scanJob.Id;

        var catalogCards = await catalogRepository.SearchAsync(
            ocrResult.DetectedName ?? ocrResult.DetectedNumber,
            ocrResult.DetectedNumber,
            null, null, 1, 50, ct);

        var candidates = matchRanking.RankMatches(ocrResult, catalogCards);
        ...
        {
            var card = catalogCards.FirstOrDefault(x => x.Id == c.CardId);
            return new
            {
                cardId = c.CardId,
                score = c.Score,
