...
/// </summary>
public class SetMatchingService : ISetMatchingService
{
    // Point budgets
    ...
    public SetMatchingService(ISetRepository repo) => _repo = repo;

    public async Task<SetMatchResult> FindBestMatchAsync(ImportedSetDto incoming, CancellationToken ct)
    {
        var candidates = await _repo.GetAllWithExternalIdsAsync(ct);
        ...
        {
            var (score, reasons) = Score(incoming, candidate);
            if (score > bestScore)
            {
                bestScore = score;
                best = candidate;
    ...
    private static (int Score, List<string> Reasons) Score(ImportedSetDto incoming, CardSet candidate)
    {
        ...
        score += namePts;
        if (namePts > 0) reasons.Add($"Name similarity {sim:P0} (+{namePts:0})");
        ...
        if (incoming.ReleaseDate.HasValue && candidate.ReleaseDate.HasValue)
        {
            ...
            score += datePts;
            if (datePts > 0) reasons.Add($"Release date within {days}d (+{datePts:0})");
        }
        ...
        if (incoming.PrintedTotal.HasValue && candidate.PrintedTotal.HasValue
            ...
            countPts = 10;
        score += countPts;
        if (countPts > 0) reasons.Add($"Card count match (+{countPts:0})");
