...
{
    public class SetImportService(ISetRepository repo, ISetMatchingService matching) : ISetImportService
    {
        public const int AutoMergeThreshold = 90;
        public const int PendingThreshold = 70;

        private readonly ISetRepository _repo = repo;
        private readonly ISetMatchingService _matching = matching;
        ...
        public async Task<CardSet> ImportSetAsync(ImportedSetDto incoming, CancellationToken ct)
        {
            ...
            CardSetExternalId? existingExt = await _repo.FindExternalIdAsync(incoming.Source, incoming.Language, incoming.ExternalId, ct);
            if (existingExt is not null)
            {
                RefreshExternalMetadata(existingExt, incoming);
                CardSet owner = await _repo.GetByIdAsync(existingExt.CardSetId, ct)
            ...
            SetMapping? mapping = await _repo.FindActiveMappingAsync(incoming.Source, incoming.Language, incoming.ExternalId, ct);
            ...
            SetMatchResult match = await _matching.FindBestMatchAsync(incoming, ct);
            ...
            CardSet newSet = await CreateCanonicalAsync(incoming, ct);
