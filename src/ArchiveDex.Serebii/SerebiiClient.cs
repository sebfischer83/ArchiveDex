...
{
    public class SerebiiClient(HttpClient httpClient)
    {
        ...
        {
            Uri url = BuildUrl("/card/english.shtml");
        ...
        {
            Uri url = BuildUrl("/card/japanese.shtml");
            var html = await _http.GetStringAsync(url, ct);
            return SerebiiParser.ParseSets(html);
        }
        ...
        {
            Uri url = BuildUrl($"/card/{Uri.EscapeDataString(setSlug)}");
            var html = await _http.GetStringAsync(url, ct);
