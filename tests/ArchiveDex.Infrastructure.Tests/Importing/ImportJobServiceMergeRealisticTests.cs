...
public class ImportJobServiceMergeRealisticTests
{
    ...
    public async Task ThreeSources_MergeIntoSingleCard_WithAllExternalIds()
    {
        ...
        var (registry, jobStore, imageStore) = CreateInfrastructure();
        ...
        await ImportFromSource(db, registry, jobStore, imageStore, "TCGdex",
            externalSetId: "sv08pt5",
            cardIdPrefix: "tcgdex/sv08pt5/en/",
            setName: SetName,
            imageUrl: false);

        await ImportFromSource(db, registry, jobStore, imageStore, "Limitless",
            externalSetId: "PRE",
            cardIdPrefix: "limitless/PRE/",
            setName: SetName,
            imageUrl: true);

        await ImportFromSource(db, registry, jobStore, imageStore, "Serebii",
    ...
    public async Task LaterSource_UpdatesNameAndRarity_OnExistingCard()
    {
        ...
        var eevee = await db.CardPrints.Include(c => c.ExternalIds)
    ...
    public async Task ThreeSources_MergeIntoOneCanonicalSet_WithThreeExternalIds()
    {
        ...
        var sets = await db.CardSets.Include(s => s.ExternalIds).ToListAsync();
        Assert.Single(sets);
    ...
    public async Task PartialMatch_CreatesPendingMapping_WhenScoreBelowAutoMerge()
    {
        ...
        var pending = await db.PendingSetMappings.ToListAsync();
    ...
    public async Task MergePreservesExistingImage_WhenAlreadyPresent()
    {
        ...
        var firstImages = await db.CardPrints.Select(c => c.ImagePath).ToListAsync();
        ...
        var afterMerge = await db.CardPrints.Select(c => c.ImagePath).ToListAsync();
    ...
    public async Task MergedCount_AccumulatesCorrectly_AcrossThreeSources()
    {
        ...
        var job1 = await ImportFromSource(db, registry, jobStore, imageStore, "TCGdex",
            externalSetId: "sv08pt5", cardIdPrefix: "tcgdex/sv08pt5/en/",
            setName: SetName, imageUrl: false);
        ...
        var job2 = await ImportFromSource(db, registry, jobStore, imageStore, "Limitless",
            externalSetId: "PRE", cardIdPrefix: "limitless/PRE/",
            setName: SetName, imageUrl: true);
        ...
        var job3 = await ImportFromSource(db, registry, jobStore, imageStore, "Serebii",
            externalSetId: "prismaticevolutions", cardIdPrefix: "serebii/prismaticevolutions/",
            setName: SetName, imageUrl: true);
    ...
    private static (FakeRegistry, FakeJobStore, FakeImageStore) CreateInfrastructure()
    {
        return (new FakeRegistry(), new FakeJobStore(), new FakeImageStore());
    }
        ...
        string? overrideRarity = null)
    {
        ...
        var service = new ImportJobService(registry, catalog, setImport, imageStore, jobStore);
    ...
    private sealed class FakeImageStore : IImageStore
    {
        ...
        {
            Interlocked.Increment(ref _storedCounter);
    ...
    /// </summary>
    private sealed class RealisticCardsDataSource : ITcgDataSource
    {
        private readonly string _setId;
        private readonly string _cardIdPrefix;
        private readonly string _setName;
        private readonly DateOnly _releaseDate;
        private readonly int _printedTotal;
        private readonly int _officialTotal;
        private readonly string _series;
        private readonly (string Number, string Name, string Rarity)[] _cardDefs;
        private readonly bool _hasImages;
        private readonly string _cardSuffix;
        private readonly string? _overrideRarity;

        public RealisticCardsDataSource(
            ...
            bool hasImages, string cardSuffix, string? overrideRarity)
        {
            SourceName = sourceName;
            _setId = setId;
            _cardIdPrefix = cardIdPrefix;
            _setName = setName;
            _releaseDate = releaseDate;
            _printedTotal = printedTotal;
            _officialTotal = officialTotal;
            _series = series;
            _cardDefs = cardDefs;
            _hasImages = hasImages;
            _cardSuffix = cardSuffix;
            _overrideRarity = overrideRarity;
        }
        ...
        public Task<IReadOnlyList<SetSummary>> GetAvailableSetsAsync(string language, CancellationToken ct = default)
        {
            ...
            return Task.FromResult<IReadOnlyList<SetSummary>>([]);
        }
        ...
        public Task<SetSummary?> GetSetMetaAsync(string setId, string language, CancellationToken ct = default)
        {
            if (!string.Equals(setId, _setId, StringComparison.OrdinalIgnoreCase))
                return Task.FromResult<SetSummary?>(null);
            return Task.FromResult<SetSummary?>(new(setId, _setName, language, _printedTotal, _officialTotal,
        ...
        public Task<IReadOnlyList<CardImportDto>> GetCardsForSetAsync(string setId, string language, CancellationToken ct = default)
        {
            ...
            foreach (var (number, name, rarity) in _cardDefs)
            {
