...
public class DomainValidationTests
{
    ...
    [InlineData(5)]
    [InlineData(100)]
    public void CollectionEntry_Quantity_GreaterThanOrEqualOne_IsValid(int quantity)
    {
    ...
    [InlineData(0)]
    [InlineData(-1)]
    public void CollectionEntry_Quantity_LessThanOne_IsInvalid(int quantity)
    {
        Assert.False(quantity >= 1);
    }
    ...
    [InlineData(9.99)]
    [InlineData(1000)]
    public void CollectionEntry_Price_NonNegative_IsValid(decimal price)
    {
        Assert.True(price >= 0);
    }
    ...
    {
        var values = Enum.GetValues<CardCondition>();
    ...
    {
        var statuses = Enum.GetValues<ScanJobStatus>();
