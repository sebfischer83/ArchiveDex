...
namespace ArchiveDex.Tcgdex.Tests
{
    public class TcgdexClientTests
    {
        ...
        [Fact]
        public void Constructor_RequiresHttpClient() => Assert.Throws<ArgumentNullException>(() => new TcgdexClient(null!));
        ...
        public async Task FetchAsync_ValidCardEndpoint_BuildsCorrectUrl()
        {
            ...
            CardResume? result = await client.FetchAsync<CardResume>(["cards", "swsh3-136"]);
        ...
        public async Task FetchAsync_WithQuery_BuildsCorrectQueryString()
        {
            ...
            Query query = new Query().Sort("name", SortOrder.ASC).Paginate(1, 20);
            var client = new TcgdexClient(CreateClient(handler));
            List<CardResume>? result = await client.FetchWithQueryAsync<CardResume>(["cards"], query);
            Assert.NotNull(result);
        }
        ...
        public async Task Endpoint_GetAsync_ReturnsItem()
        {
            ...
            var client = new TcgdexClient(CreateClient(handler));
            Set? set = await client.Sets.GetAsync("swsh3");
            Assert.NotNull(set);
        ...
        public async Task FetchSetsAsync_WithSerie_ReturnsSetsFromSerie()
        {
            ...
            var client = new TcgdexClient(CreateClient(handler));
            _ = Assert.Single((await client.FetchSetsAsync("swsh"))!);
        }

        [Fact]
        public void Query_CombinesMultipleFilters()
        {
            Query q = new Query().Contains("name", "charizard").Sort("name", SortOrder.DESC).Paginate(1, 10);
        ...
        {
            var q = new Query();
            _ = q.Not.Equal("name", "pikachu");
            _ = q.Not.IsNull("rarity");
        ...
        {
            Query q = new Query().GreaterThan("hp", 100).LesserOrEqualThan("hp", 200);
        ...
        public async Task Language_IsIncludedInUrl()
        {
            ...
            _ = await client.FetchAsync<CardResume>(["cards", "x"]);
        }
        ...
        public void WithLanguage_SharesHttpClient()
        {
            ...
            TcgdexClient deClient = client.WithLanguage(SupportedLanguage.de);
